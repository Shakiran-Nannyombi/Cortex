    ...
    <div className={`min-h-screen flex ${isDark ? 'bg-blue-950' : 'bg-white'}`}>
      ...
      >
        ...
        <div className="relative space-y-8">
          <div>
            <h2 className="text-3xl font-bold text-white mb-3">Your documents, <br />intelligently organized.</h2>
            <p className="text-blue-100 text-lg">Sign in to access your AI-powered document workspace.</p>
          </div>
        ...
        <p className="relative text-blue-200 text-sm">© {new Date().getFullYear()} Cortex · Microsoft DevDays 2026 Hackathon</p>
      </div>
      ...
      <div className={`w-full lg:w-1/2 flex flex-col items-center justify-center px-6 py-16 relative ${isDark ? 'bg-blue-950' : 'bg-gray-50'}`}>
        <div className="absolute top-8 left-8">
          <Link to="/" className={`flex items-center gap-2 text-sm font-medium transition-colors ${isDark ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-gray-900'}`}>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className={isDark ? 'text-gray-400' : 'text-gray-500'}>Sign in to your Cortex account</p>
          </div>
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className={`${inputClass} pl-10`} placeholder="you@example.com" />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} required className={`${inputClass} pl-10`} placeholder="••••••••" />
              </div>
            </div>
            <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 bg-blue-600 text-white py-3 px-4 rounded-xl font-semibold hover:bg-blue-700 disabled:opacity-50 transition-all duration-200 text-sm">
              {loading ? 'Signing in...' : <><span>Sign In</span><ArrowRight className="w-4 h-4" /></>}
            </button>
            <button type="button" onClick={handleDemoLogin} disabled={loading} className={`w-full py-3 px-4 rounded-xl font-semibold transition-all duration-200 text-sm ${isDark ? 'bg-gray-700 text-white hover:bg-gray-600' : 'bg-gray-200 text-gray-900 hover:bg-gray-300'}`}>
              {loading ? 'Loading...' : 'Try Demo Account'}
            </button>
          </form>
          <p className={`mt-6 text-center text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
            Don't have an account?{' '}
            <Link to="/register" className="text-blue-600 hover:text-blue-500 font-semibold">Create one free</Link>
          </p>
        </div>
      </div>
    </div>
